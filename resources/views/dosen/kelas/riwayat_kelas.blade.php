...
<div class="mb-6 flex items-center justify-between">
  <div class="flex items-center gap-3">
    <a href="{{ route('dosen.kelas_riwayat') }}"
       ...
       class="flex items-center justify-center w-9 h-9 rounded-lg border bg-blue-600 text-white j">
...
<div class="mb-6">
  <div class="flex items-center gap-2 rounded-xl bg-white p-1 shadow w-fit">
    <a href="{{ route('dosen.kelas_riwayat.detail', $kelas->id) }}?tab=materi"
    ...
    <a href="{{ route('dosen.kelas_riwayat.detail', $kelas->id) }}?tab=tugas"
       class="px-4 py-2 text-sm font-semibold rounded-lg {{ $tab === 'tugas' ? 'bg-blue-800 text-white shadow' : 'text-gray-600 hover:bg-gray-100' }}">
      Tugas
    </a>
    <a href="{{ route('dosen.kelas_riwayat.detail', $kelas->id) }}?tab=ujian"
       class="px-4 py-2 text-sm font-semibold rounded-lg {{ $tab === 'ujian' ? 'bg-blue-800 text-white shadow' : 'text-gray-600 hover:bg-gray-100' }}">
      Ujian
    </a>
    <a href="{{ route('dosen.kelas_riwayat.detail', $kelas->id) }}?tab=rekap"
       class="px-4 py-2 text-sm font-semibold rounded-lg {{ $tab === 'rekap' ? 'bg-blue-800 text-white shadow' : 'text-gray-600 hover:bg-gray-100' }}">
      Rekap
    </a>
  </div>
</div>
...
@else
  @include('dosen.materi._riwayat_content', [
    'kelas' => $kelas,
