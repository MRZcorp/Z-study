...
<div class="p-6 bg-gray-100 min-h-screen">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="bg-white rounded-2xl shadow p-6">
        <div class="flex flex-col items-center text-center">
          ...
          <h3 class="mt-4 text-lg font-semibold">
            {{ $nama ?? ($user->name ?? '-') }} {{ $gelar ?? '' }}
          </h3>

          <form
            action="{{ route('dosen.pengaturan.update') }}"
            method="POST"
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow p-6">
          ...
          <form id="infoForm" action="{{ route('dosen.pengaturan.update.info') }}" method="POST" class="mt-6">
            @csrf
            @method('PUT')
        ...
        <div class="bg-white rounded-2xl shadow p-6">
          ...
          <form id="securityForm" action="{{ route('dosen.pengaturan.update.password') }}" method="POST" class="hidden mt-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
  ...
  <script>
    const dosenFotoPreview = document.getElementById('dosenFotoPreview');
    const dosenFotoInput = document.getElementById('dosenFotoInput');

    if (dosenFotoPreview && dosenFotoInput) {
      dosenFotoPreview.addEventListener('click', () => dosenFotoInput.click());
      dosenFotoInput.addEventListener('change', () => {
        if (dosenFotoInput.files && dosenFotoInput.files.length > 0) {
          dosenFotoInput.closest('form').submit();
        }
      });
    }
