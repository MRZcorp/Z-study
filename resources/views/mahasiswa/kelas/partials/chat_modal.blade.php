    ...
    <div id="contactPanel" class="absolute inset-y-0 right-0 z-40 w-[340px] max-w-[90%] translate-x-full bg-gradient-to-b from-[#f5f8fb]/80 to-[#e7edf3]/80 border-l border-slate-300 shadow-xl transition-transform duration-300 ease-out">
      ...
      <div class="p-6 text-center">
        ...
        <div class="mt-5 rounded-xl border border-slate-200 bg-white/80 p-4 text-left shadow-sm space-y-2">
          <div>
            <p id="contactPanelField1Label" class="text-xs uppercase tracking-wide text-slate-400">Homebase</p>
            <p id="contactPanelField1Value" class="text-sm font-medium text-slate-700">-</p>
          </div>
          <div>
            <p id="contactPanelField2Label" class="text-xs uppercase tracking-wide text-slate-400">Mata Kuliah Diampu</p>
...
<script>
  let chatUserMap = {};
  let chatDosenContact = {};
  const currentUserId = Number(@json(session('user_id')));
  const currentUserName = @json(session('name') ?? '-');
  const defaultChatBaseUrlTemplate = @json(route('mahasiswa.kelas.diskusi.index', ['kelas' => '__KELAS_ID__']));
  const defaultChatMessageUrlTemplate = @json(route('mahasiswa.kelas.diskusi.update', ['kelas' => '__KELAS_ID__', 'diskusi' => '__DISKUSI_ID__']));
  const unreadStatusUrl = window.__chatUnreadStatusUrl || @json(route('mahasiswa.diskusi.unread_status'));
  const markReadUrl = window.__chatMarkReadUrl || @json(route('mahasiswa.diskusi.mark_read'));
  ...
  function getDosenContact() {
    const list = Object.values(chatUserMap || {});
    return list.find((c) => c?.role === 'dosen') || {};
  }
  ...
  function resolveContactFromMessage(item) {
    ...
    const dosen = getDosenContact();
    const normalizedName = String(item.user_name || '').toLowerCase().trim();
    const normalizedDosen = String(dosen?.name || '').toLowerCase().trim();

    if (byId?.role === 'dosen') return byId;
    if (byName?.role === 'dosen') return byName;
    if (normalizedName && normalizedDosen && normalizedName === normalizedDosen) return dosen;
  ...
  function updateMineMessageOffsets() {
    ...
    const baseComposerPad = 16; // p-4 from tailwind
    ...
    wrapper.innerHTML = `
      <div class="flex items-start gap-1 max-w-[92%] md:max-w-[82%]">
        ${!isMine ? `
          ...
          >
            <img
            src="${escapeHtml(contact.foto || (String(item.user_name || '') === String(chatDosenContact.name || '') ? chatDosenContact.foto : '') || item.user_foto || defaultAvatar)}"
            alt="${senderName}"
  ...
  function applyChatContext(button) {
    ...
    chatDosenContact = {
      name: button?.dataset?.dosenName || '',
      foto: button?.dataset?.dosenFoto || defaultAvatar,
      phone: button?.dataset?.dosenPhone || '-',
      role: 'dosen',
      gelar: button?.dataset?.dosenGelar || '',
      homebase: button?.dataset?.dosenHomebase || '-',
      mata_kuliah: button?.dataset?.dosenMatkul || '-',
    };
    closeContactPanel();
  ...
  function openContactPanel(contact) {
    ...
    const roleLabel = isMahasiswa ? 'Mahasiswa' : 'Dosen Pengampu';
    ...
    if (field1LabelEl) field1LabelEl.textContent = isMahasiswa ? 'Fakultas' : 'Homebase Fakultas';
    if (field1ValueEl) field1ValueEl.textContent = isMahasiswa ? (contact?.fakultas || '-') : (contact?.homebase || '-');
    if (field2LabelEl) field2LabelEl.textContent = isMahasiswa ? 'Prodi' : 'Mata Kuliah Diampu';
    if (field2ValueEl) field2ValueEl.textContent = isMahasiswa ? (contact?.prodi || '-') : (contact?.mata_kuliah || '-');
  ...
  document.getElementById('chatMessages')?.addEventListener('click', function (e) {
    ...
    if (openContactBtn) {
      ...
      if (!contact?.name || String(contact.name) === String(currentUserName)) {
        const dosenContact = getDosenContact()?.name ? getDosenContact() : chatDosenContact;
        if (dosenContact?.name) {
          contact = dosenContact;
        }
      }
  ...
  window.addEventListener('resize', updateMineMessageOffsets);
  refreshUnreadDots();
