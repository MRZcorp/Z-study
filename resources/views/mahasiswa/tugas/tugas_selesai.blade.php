...
@php
  ...
  $tugas_selesai = ($tugas_kelas ?? collect())->filter(function ($tugas) use ($now) {
  ...
  if (request('matkul_id')) {
      $tugas_selesai = $tugas_selesai->filter(function ($tugas) {
...
<div class="p-6 bg-gray-100 min-h-screen">
  ...
  <div class="mb-6 flex items-center justify-between">
    <div class="flex items-center gap-2 rounded-xl bg-white p-1 shadow w-fit">
      <a href="{{ url('/mahasiswa/tugas') }}" class="px-4 py-2 text-sm font-semibold rounded-lg text-gray-600 hover:bg-gray-100">Ditugaskan</a>
      <span class="px-4 py-2 text-sm font-semibold rounded-lg bg-blue-800 text-white shadow">Selesai</span>
    </div>
...
<script>
  ...
  const showDosenFiles = () => {
    isMhsMode = false;
    previewDeskripsi.textContent = dosenDesc || '-';
    previewSubmitted.textContent = '-';
    previewSubmittedWrap?.classList.add('hidden');
    previewFileList.innerHTML = '';
    if (dosenFiles.length === 0) {
      previewFileList.innerHTML = '<span class="text-slate-400 text-sm">Tidak ada file.</span>';
      setActiveFile(null);
      return;
    }
    dosenFiles.forEach((file, idx) => {
    ...
    setActiveFile(dosenFiles[0]);
  };

  const showMhsFile = () => {
    isMhsMode = true;
    previewDeskripsi.textContent = mhsDesc || '-';
    previewSubmitted.innerHTML = submittedText
      ? `<span class="text-emerald-600 font-semibold">${submittedText}</span>`
      : '-';
    previewSubmittedWrap?.classList.remove('hidden');
    previewFileList.innerHTML = '';
    if (!mhsFile) {
      previewFileList.innerHTML = '<span class="text-slate-400 text-sm">Belum ada file mahasiswa.</span>';
      setActiveFile(null);
      return;
    }
    const btnFile = document.createElement('button');
    btnFile.type = 'button';
    btnFile.className = 'text-left hover:underline';
    btnFile.textContent = mhsFile.name || 'File mahasiswa';
    btnFile.addEventListener('click', () => setActiveFile(mhsFile));
    previewFileList.appendChild(btnFile);
    setActiveFile(mhsFile);
  };

  document.querySelectorAll('.btn-preview-tugas').forEach((btn) => {
    btn.addEventListener('click', () => {
      dosenFiles = JSON.parse(btn.dataset.files || '[]');
      const mhsUrl = btn.dataset.mhsFile || '';
      const mhsName = btn.dataset.mhsName || '';
      dosenDesc = btn.dataset.deskripsi || '-';
      mhsDesc = btn.dataset.mhsDesc || '-';
      submittedText = btn.dataset.mhsSubmitted || '';
      mhsFile = mhsUrl ? { name: mhsName, url: mhsUrl, ext: (mhsName.split('.').pop() || '') } : null;
  ...
  btnToggleMhs?.addEventListener('click', () => {
    if (isMhsMode) {
      if (mhsFile?.url) {
        window.open(mhsFile.url, '_blank');
      }
      return;
    }
    showMhsFile();
